...
import { ProviderType } from "../screens/(main)/hisstory/providers/new";
...
// =======================Providers=========================
export const addProviders = async (newProvider: ProviderType) => {
    const documentId = await getUserId();
    ...
    try {
        ...
        if (existingDoc.exists()) {
            const existingProviders = existingDoc.data()?.providers || [];
